...
def Key_Stats(gather=["Total Debt/Equity",
	...
	stock_df = pd.DataFrame.from_csv("stock_prices_done.csv")
			...
			for file in each_file:
				...
				try:
					...
					for each_data in gather:
						try:
							regex = re.escape(each_data) + r'.*?(\d{1,8}\.\d{1,8}M?B?|N/A)%?</td>' # Grab data from html.
					...
					try:
						...
						try:
							...
							print("sp500 1 year later error",str(e))


					# Obtain next year's stock data	
					try:
					...
					# If Difference Outperforms S&P by 5%
					if difference > 5:
						status = 1
					else:
						status = 0
