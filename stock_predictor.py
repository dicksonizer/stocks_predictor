...
def Build_Data_Set():
	...
	data_df = data_df.replace("N/A",0)#.replace("NaN",-999)
	data_df = data_df.fillna(0)

	data_df["Status2"] = list(map(Status_Calc, data_df["stock_p_change"], data_df["sp500_p_change"]))

	# Take in parameters in array
	X = np.array(data_df[FEATURES].values)#.tolist())
	# X = np.nan_to_num(X)

	# Normalize "outperform" and "underperform" strings 
...
def Analysis():
	...
	# print("Average market return", str(avg_market)+'% more')
...
"""IGNORE BELOW (PERSONAL LEARNING NOTES)"""
	# #Graph, to check out correlation between 2 features
	# w = clf.coef_[0]
	...
	# # NOTETOSELF: X[:, 1] is first element of each element in numpy array. The whole equation pairs the x's and y's easily without a for loop
